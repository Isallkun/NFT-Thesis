...
class NFTService {
  ...
  async mintNFT(metadataUri, recipientWallet, certificateData) {
    try {
      ...
      const mintSignature = await sendAndConfirmTransaction(this.connection, transaction, [this.payer], { commitment: "confirmed" });
      ...
      const metadataSignature = await sendAndConfirmTransaction(this.connection, metadataTransaction, [this.payer], { commitment: "confirmed" });
